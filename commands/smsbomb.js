...
module.exports = async function(sock, chatId, message, q) {
    try {
        ...
        if (!q) return await sock.sendMessage(chatId, { text: '⚠️ .smsbomb 923xxxxxxxxxx' }, { quoted: message });
        ...
        for (let batch = 0; batch < 5; batch++) {
            ...
            if (batch < 4) await new Promise(r => setTimeout(r, 200));
        }

        await sock.sendMessage(chatId, { 
            text: `✅ SMS BOMBING DONE\n👤 Target: +${target}\n💣 Sent: ${sent}/50\n⚡ Speed: ULTRA FAST` 
        }, { quoted: message });
    } catch(err) { 
